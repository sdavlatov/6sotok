...
export default async function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--paper)]">
      ...
      {/* ═══ HERO ═══ */}
      <section className="relative border-b border-[var(--line-soft)] bg-white">
        ...
        <div className="relative max-w-[1440px] mx-auto px-4 sm:px-6 pt-10 sm:pt-16 pb-6 sm:pb-8">
          <div className="max-w-5xl mb-7 sm:mb-10">
            ...
            <p className="mt-5 sm:mt-7 text-[15px] sm:text-[18px] text-[var(--ink-500)] leading-snug max-w-2xl tracking-tight">
      ...
      {/* ═══ AUDIENCE BLOCKS ═══ */}
      <section className="border-t border-[var(--line-soft)] bg-white">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 py-10 sm:py-20">
          <div className="grid lg:grid-cols-2 gap-4 sm:gap-5">

            {/* Покупателям */}
            <Link href="/catalog" className="tile group relative overflow-hidden rounded-2xl sm:rounded-3xl bg-[var(--paper-2)] border border-[var(--line)] sm:p-10 sm:min-h-[440px] flex flex-col justify-between">
              ...
              <div className="drift hidden sm:block absolute top-7 right-7 w-44 h-44 rounded-2xl map-bg border border-[var(--line-soft)] shadow-[var(--sh-2)] overflow-hidden pointer-events-none">
              ...
              <div className="sm:hidden relative map-bg overflow-hidden" style={{ height: 160 }}>
                ...
                <div className="absolute inset-x-0 bottom-0 h-12 pointer-events-none"
                  style={{ background: 'linear-gradient(to top, var(--paper-2) 0%, transparent 100%)' }} />
                <div className="absolute bottom-3 right-4 flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-primary" />
                  <span className="text-[11px] font-mono text-[var(--ink-400)]">{landCount} участков</span>
                </div>
              </div>
              ...
              <div className="relative p-6 sm:p-0">
                ...
                <h3 className="mt-3 sm:mt-4 font-black tracking-[-0.04em] text-[34px] sm:text-[52px] leading-[0.95] text-[var(--ink-900)]">
                  Ищите участки<br />прямо<br />на карте.
                </h3>
              </div>
              <div className="relative p-6 pt-0 sm:p-0">
                <p className="text-[13.5px] sm:text-[14.5px] text-[var(--ink-500)] leading-snug max-w-md">
                ...
                <div className="mt-5 sm:mt-7 flex items-center gap-3 flex-wrap">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 text-[11.5px] sm:text-[12px] text-[var(--ink-400)] font-mono">
                    <span><b className="text-[var(--ink-900)]">{landCount.toLocaleString('ru-RU')}</b> объявл.</span>
                    <span className="text-zinc-300">·</span>
                    <span><b className="text-[var(--ink-900)]">{locations.length}</b> областей</span>
                  </div>
                </div>
              </div>
            </Link>
            ...
            <Link href="/add-listing" className="tile group relative overflow-hidden rounded-2xl sm:rounded-3xl bg-[#021A0E] text-white p-6 sm:p-10 min-h-[320px] sm:min-h-[440px] flex flex-col justify-between">
              ...
              <div className="relative">
                ...
                <div className="mt-5 sm:mt-7">
                  <span className="px-4 sm:px-5 h-10 sm:h-11 rounded-xl bg-white text-[var(--ink-900)] font-semibold text-[13px] sm:text-[13.5px] tracking-tight inline-flex items-center gap-2 group-hover:bg-primary group-hover:text-white transition">
      ...
      {/* ═══ BUSINESS SECTION ═══ */}
      <section className="bg-[var(--paper)] border-t border-[var(--line-soft)]">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 py-10 sm:py-20">
          ...
          <div className="flex items-start sm:items-end justify-between gap-4 sm:gap-6 flex-wrap mb-7 sm:mb-10">
            <div>
              ...
              <h2 className="font-black tracking-[-0.05em] text-[38px] sm:text-[56px] md:text-[64px] leading-[0.95] text-[var(--ink-900)]">
                Готовый бизнес.<br />
                <span className="text-[var(--ink-400)]">Не только земля.</span>
              </h2>
              <p className="mt-4 sm:mt-5 text-[14px] sm:text-[16px] text-[var(--ink-500)] leading-snug tracking-tight max-w-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-5">
            {businessPreview.map((l, idx) => (
              <Link key={l.id} href={`/business/${l.slug}`}
                className="tile group bg-white rounded-3xl border border-[var(--line)] shadow-[var(--sh-1)] overflow-hidden hover:shadow-[var(--sh-2)] transition">
                ...
                <div className="p-6">
                  <div className="text-[11px] font-medium text-[var(--ink-400)] uppercase tracking-wider">{l.landType ?? 'Готовый бизнес'}</div>
                  <h3 className="mt-1.5 font-black tracking-[-0.035em] text-[22px] leading-tight text-[var(--ink-900)] line-clamp-2">{l.title}</h3>
                  <div className="mt-4 pt-4 border-t border-[var(--line-soft)] grid grid-cols-3 gap-3">
                    <div>
                      <div className="text-[10px] font-mono uppercase tracking-wider text-[var(--ink-400)]">Цена</div>
                      <div className="mt-0.5 font-black tracking-[-0.035em] text-[16px] text-[var(--ink-900)]">
                        {l.price ? `${fmtM(l.price)} млн ₸` : '—'}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] font-mono uppercase tracking-wider text-[var(--ink-400)]">Выручка</div>
                      <div className="mt-0.5 font-black tracking-[-0.035em] text-[16px] text-[var(--ink-900)]">
                        {l.monthlyRevenue ? `${fmtM(l.monthlyRevenue)} м/мес` : '—'}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] font-mono uppercase tracking-wider text-[var(--ink-400)]">Окуп.</div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-2 text-[12.5px]">
            <span className="text-[var(--ink-400)] mr-1 font-medium">Категории:</span>
            {[
              ...
              <Link key={label} href={href}
                className="px-3.5 h-9 rounded-full bg-white border border-[var(--line)] text-[var(--ink-700)] hover:border-[var(--ink-900)] hover:text-[var(--ink-900)] transition flex items-center">
                {label}
              </Link>
            ))}
          </div>

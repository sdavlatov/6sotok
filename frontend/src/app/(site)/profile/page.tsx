...
const STATUS_CLASS: Record<string, string> = {
  draft: 'bg-[var(--paper-2)] text-[var(--ink-400)]',
...
function ListingCard({ listing, onDelete, onStatusChange }: {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[var(--line-soft)] shadow-[0_2px_8px_rgba(0,0,0,0.05)] overflow-hidden">
      {/* Thumbnail */}
      <div className="relative aspect-[4/3] bg-[var(--paper-2)] overflow-hidden">
        {thumb ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            {isBusiness
              ? <Briefcase className="size-8 text-[var(--ink-300)]" />
              : <TreePine className="size-8 text-[var(--ink-300)]" />
            }
          </div>
        )}
        ...
        <Link href={editHref}
          ...
          <Edit3 className="size-3.5 text-[var(--ink-500)]" />
        </Link>
      </div>

      {/* Info */}
      <div className="p-4 space-y-1.5">
        <p className="font-semibold text-[var(--ink-900)] text-sm leading-snug line-clamp-2">{listing.title}</p>
        <p className="text-base font-bold text-[var(--ink-900)]">{fmt(listing.price)}</p>
        <div className="flex items-center gap-3 text-xs text-[var(--ink-400)] pt-0.5">
          {area && <span>{area}</span>}
          {listing.views != null && (
      ...
      <div className="px-4 pb-4 flex items-center gap-3 border-t border-zinc-50 pt-3">
        {!isSold && (
          <button onClick={() => handleStatusChange('sold')} disabled={changingStatus}
            className="flex items-center gap-1 text-xs font-semibold text-[var(--ink-400)] hover:text-amber-600 transition-colors disabled:opacity-40">
        ...
        {isSold && (
          <button onClick={() => handleStatusChange('draft')} disabled={changingStatus}
            className="flex items-center gap-1 text-xs font-semibold text-[var(--ink-400)] hover:text-primary transition-colors disabled:opacity-40">
            <CheckCircle2 className="size-3.5" />
            Снять с продажи
          </button>
        )}
        <button onClick={handleDelete} disabled={deleting}
          className="ml-auto flex items-center gap-1.5 text-xs font-medium text-[var(--ink-300)] hover:text-red-500 transition-colors disabled:opacity-40">
...
export default function ProfilePage() {
  ...
  if (loading || !user) {
    return (
      <div className="min-h-[calc(100vh-80px)] bg-[var(--paper)] flex items-center justify-center">
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-[var(--paper)]">
      <Container>
        <div className="max-w-4xl mx-auto py-8 space-y-5">

          {/* ── Профиль ────────────────────────────────────────────────────── */}
          <div className="bg-white rounded-2xl border border-[var(--line-soft)] shadow-[0_2px_8px_rgba(0,0,0,0.05)] overflow-hidden">
            <div className="bg-gradient-to-br from-primary-soft via-white to-white px-6 pt-6 pb-5 border-b border-[var(--line-soft)]">
              <div className="flex items-center gap-4">
                <div className="size-14 rounded-2xl bg-primary flex items-center justify-center shrink-0">
                  <span className="text-lg font-bold text-white">{initials}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <h1 className="text-lg font-bold text-[var(--ink-900)] truncate">{user.name}</h1>
                ...
                {!editing && (
                  <button onClick={startEdit} className="size-9 flex items-center justify-center rounded-xl bg-[var(--paper-2)] hover:bg-[var(--paper-3)] text-[var(--ink-500)] transition-colors shrink-0">
            ...
            {editing ? (
              <form onSubmit={save} className="p-5 space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-[var(--ink-700)] mb-1.5">Имя</label>
                    <input type="text" value={name} onChange={e => setName(e.target.value)} required
                      className="w-full px-4 py-2.5 bg-white border border-[var(--line)] rounded-xl text-[15px] focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 transition-all" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[var(--ink-700)] mb-1.5">Телефон</label>
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button type="button" onClick={cancelEdit}
                    className="flex items-center gap-2 bg-[var(--paper-2)] hover:bg-[var(--paper-3)] text-[var(--ink-700)] font-medium px-4 py-2 rounded-xl text-sm transition-colors">
              ...
              <div className="px-6 py-4 flex flex-wrap gap-x-8 gap-y-2">
                <div className="flex items-center gap-2 text-sm text-[var(--ink-500)]">
                  <Mail className="size-4 text-[var(--ink-400)]" />{user.email}
                </div>
                {user.phone && (
                  <div className="flex items-center gap-2 text-sm text-[var(--ink-500)]">
                    <Phone className="size-4 text-[var(--ink-400)]" />{user.phone}
                  </div>
                )}
                {user.isAgency && (
                  <div className="flex items-center gap-2 text-sm text-[var(--ink-500)]">
                    <Building2 className="size-4 text-[var(--ink-400)]" />Агентство недвижимости
                  </div>
                )}
              </div>
            )}
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={s.label} className="bg-white rounded-2xl border border-[var(--line-soft)] shadow-[0_2px_8px_rgba(0,0,0,0.04)] p-4 text-center">
                <p className="text-2xl font-black text-[var(--ink-900)]">{s.value}</p>
                <p className="text-xs text-[var(--ink-400)] mt-0.5">{s.label}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[var(--line-soft)] shadow-[0_2px_8px_rgba(0,0,0,0.05)] overflow-hidden">
            {/* Табы */}
            <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[var(--line-soft)]">
              <div className="flex gap-1 bg-[var(--paper-2)] rounded-xl p-1">
                <button onClick={() => setTab('land')}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${tab === 'land' ? 'bg-white shadow-sm text-[var(--ink-900)]' : 'text-[var(--ink-400)] hover:text-[var(--ink-700)]'}`}>
                  <TreePine className="size-4" />
                  Земля
                  {landListings.length > 0 && (
                    <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded-full ${tab === 'land' ? 'bg-primary-soft text-primary' : 'bg-[var(--paper-3)] text-[var(--ink-400)]'}`}>
                      {landListings.length}
                    </span>
                  )}
                </button>
                <button onClick={() => setTab('business')}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${tab === 'business' ? 'bg-white shadow-sm text-[var(--ink-900)]' : 'text-[var(--ink-400)] hover:text-[var(--ink-700)]'}`}>
                  <Briefcase className="size-4" />
                  Бизнес
                  {bizListings.length > 0 && (
                    <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded-full ${tab === 'business' ? 'bg-primary-soft text-primary' : 'bg-[var(--paper-3)] text-[var(--ink-400)]'}`}>
                      {bizListings.length}
                    </span>
                  )}
                </button>
            ...
            <div className="p-5">
              {listingsLoading ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {[1, 2, 3].map(i => (
                    <div key={i} className="rounded-2xl bg-[var(--paper-2)] animate-pulse aspect-[3/4]" />
                  ))}
                </div>
              ) : tab === 'land' ? (
                landListings.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-14 text-center">
                    <div className="bg-[var(--paper-2)] p-4 rounded-2xl mb-3">
                      <TreePine className="size-8 text-[var(--ink-400)]" />
                    </div>
                    <p className="text-base font-semibold text-[var(--ink-700)] mb-1">Нет объявлений по земле</p>
                    <p className="text-sm text-[var(--ink-400)] mb-4">Разместите первый участок прямо сейчас</p>
                ...
                bizListings.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-14 text-center">
                    <div className="bg-[var(--paper-2)] p-4 rounded-2xl mb-3">
                      <Briefcase className="size-8 text-[var(--ink-400)]" />
                    </div>
          ...
          <div className="flex items-center gap-4 px-1">
            ...
            <p className="text-xs text-[var(--ink-400)] ml-auto">Объявления видны после публикации</p>
          </div>

          {/* ── Выход ──────────────────────────────────────────────────────── */}
          <button onClick={signOut}
            className="w-full flex items-center justify-center gap-2 bg-white hover:bg-red-50 text-[var(--ink-400)] hover:text-red-600 border border-[var(--line-soft)] hover:border-red-100 font-medium px-5 py-3 rounded-2xl transition-all text-sm">

...
export function ContactCard({ price, pricePerSotka, seller }: ContactCardProps) {
  const formattedPrice = new Intl.NumberFormat('ru-RU').format(price);
  const formattedPerSotka = new Intl.NumberFormat('ru-RU').format(pricePerSotka);

  return (
    <div className="sticky top-28 rounded-3xl border border-zinc-200 p-6 md:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-white">
      ...
      <div className="mt-8 space-y-3">
        <button className="w-full rounded-2xl bg-primary py-4 font-extrabold text-white transition-all hover:bg-primary-light hover:-translate-y-0.5 active:scale-95 shadow-lg shadow-primary/20 flex items-center justify-center gap-2">
        ...
        <button className="w-full rounded-2xl bg-[#25D366] py-4 font-extrabold text-white transition-all hover:bg-[#20BE5A] hover:-translate-y-0.5 active:scale-95 shadow-lg shadow-[#25D366]/20 flex items-center justify-center gap-2">
